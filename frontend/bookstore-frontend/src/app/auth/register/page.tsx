...
export default function RegisterPage() {
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] grid-bg flex items-center justify-center px-4 py-12">
      <Link href="/" className="fixed top-6 left-6 text-sm text-zinc-500 hover:text-white transition-colors">
        ← Home
      </Link>
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-8">
          ...
          <p className="text-sm text-zinc-500 mb-8">Join 120K+ readers today.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  placeholder="Alex"
                  className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-2.5 text-white text-sm placeholder:text-zinc-600 focus:outline-none focus:border-green-400/50 transition-all"
                />
              </div>
              <div>
            ...
            <div>
              <label className="block text-xs text-zinc-400 mb-1.5 font-medium">Phone <span className="text-zinc-600">(optional)</span></label>
              <input
                type="tel"
                value={form.phone}
            ...
            <label className="flex items-start gap-3 cursor-pointer group">
              <div
                ...
                onClick={() => set("marketingConsent", !form.marketingConsent)}
              >
                {form.marketingConsent && <span className="text-black text-xs font-bold">✓</span>}
              </div>
              <span className="text-xs text-zinc-500 leading-relaxed">
                Send me updates about new books, events and exclusive drops
              </span>
            </label>
